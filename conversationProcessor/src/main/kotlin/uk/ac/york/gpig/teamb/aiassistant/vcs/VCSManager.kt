...
import uk.ac.york.gpig.teamb.Repository
...
//        }
    ...
    fun processChanges(
        ...
        val branchName = "${issue.number}-${issue.title.toString().lowercase().replace(" ", "-")}"
        gitFacade.createBranch(gitFile, branchName)

        logger.info("Created branch $branchName, fetching file tree...")
        val fileTree = retrieveFileTree(repository.fullName.toString(), "main")
