 ...
 *
 * Since we are not using tool calls or function calls, we will omit the `tool_calls` and
 * `function_call` options for now.
 */
enum class FinishReason {
    ...
    /**
     * Response was longer than allowed (will happen if we set a max response size in tokens and the
     * model exceeds it)
     */
    @JsonProperty("length")
    LENGTH,
