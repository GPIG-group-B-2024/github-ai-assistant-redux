...
class C4NotationReadFacade(
     ...
     *
     * Note: returns all components in a flat list, children and parents need to be re-organised in a
     * later step
     */
    ...
    fun getRelationships(workspaceId: UUID): List<C4RelationshipEntity> =
        ctx
            ...
            .from(RELATIONSHIP)
            .join(fromMember)
            .on(RELATIONSHIP.START_MEMBER.eq(fromMember.ID))
            .join(toMember)
            .on(RELATIONSHIP.END_MEMBER.eq(toMember.ID))
    ...
    fun getRepoId(repoName: String): UUID? =
        ctx
            ...
            .where(GITHUB_REPOSITORY.FULL_NAME.eq(repoName))
            .fetchOne(GITHUB_REPOSITORY.ID)

    fun getRepoUrl(repoName: String): String? =
        ctx
            .selectFrom(GITHUB_REPOSITORY)
            .where(GITHUB_REPOSITORY.FULL_NAME.eq(repoName))
            .fetchOne(GITHUB_REPOSITORY.URL)
}
