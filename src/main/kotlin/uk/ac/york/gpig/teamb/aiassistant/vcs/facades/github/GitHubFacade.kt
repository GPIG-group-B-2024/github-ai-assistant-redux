...
/**
 * Interacts with the GitHub API for GitHub-specific things like pull requests, comments, issues
 * etc.
 */
@Service
class GitHubFacade(
    @Value("\${app_settings.github_app_key}") val githubKeyFileContents: String,
) {
    private val logger = LoggerFactory.getLogger(this::class.java)

    /**
     * We use a different endpoint address for mocking github requests in testing. In production, use
     * the normal github endpoint
     */
    ...
    fun createPullRequest(
        ...
        logger.info(
            "Successfully created pull request ${pullRequest.number} in repository ${repo.name}",
        )
    }
    ...
    fun retrieveBlobs(
        ...
        withAuthInRepo(repoName) { repo ->
            paths.map { path ->
                ...
                val fileContents =
                    repo
                        .getFileContent(path, ref) // make request...
                        .read() // get bytes...
    ...
    fun generateInstallationToken(): String =
        GitHubBuilder()
            ...
            .create() // These tokens are short-lived, so for now, create a new one for each action.
            // TODO: look into caching
            .token

    /**
     * Authenticate, checkout the repository with a given name, do some stuff with it and return the
     * output (if any)
     *
    ...
    internal fun <R> withAuthInRepo(
        ...
        val github =
            GitHubBuilder().withEndpoint(githubEndpoint).withAppInstallationToken(token).build()
