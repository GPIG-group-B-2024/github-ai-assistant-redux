...
class LLMConversationReadFacade(
    ...
    fun listConversationMessages(conversationId: UUID): List<LLMMessageEntity> =
        ctx
            .select(LLM_MESSAGE.ID, LLM_MESSAGE.CONTENT, LLM_MESSAGE.CREATED_AT, LLM_MESSAGE.ROLE)
            .from(LLM_MESSAGE)
    ...
    /**
     * Get a conversation associated with a given issue in a given repository. NOTE: assumes we only
     * have one conversation per issue.
     */
    fun fetchConversation(
        ...
        ctx
            ...
            ).where(LLM_CONVERSATION.REPO_ID.eq(repoId).and(LLM_CONVERSATION.ISSUE_ID.eq(issueId)))
     ...
     *
     * NOTE: this will probably need to be refactored if we plan to have multiple conversations per
     * issue. Will have to discuss implementations in a team meeting if this is the case.
     */
    fun checkConversationExists(
        repoId: UUID,
        issueId: Int,
    ) = ctx.fetchExists(
        LLM_CONVERSATION.where(
            LLM_CONVERSATION.REPO_ID.eq(repoId).and(LLM_CONVERSATION.ISSUE_ID.eq(issueId)),
        ),
    )
}
